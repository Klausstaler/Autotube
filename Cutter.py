...
class VideoSetup:
    ...
    def create_video(self, file_path, ID):
        ...
        os.system(
            f"balcon -n \"{self.voice_vol}\" -t \"r slash ask reddit by Robotainment\" -v {self.voice_vol} -w \"tmp"
            f"/files/0.wav\"")
        ...
        os.system(
            f"balcon -n \"{self.voice_vol}\" -t \"Like and subscribe for good luck, you handsome gentleman.   ....\" -v {self.voice_vol} -w \"outro.wav\"")
    ...
    def create_audio_img(self, val, concat):
        if val[0] == -1:
            ...
            os.system(f"balcon -n \"{self.voice_vol}\" -t \"{val[1]}\" -v {self.voice_vol} -w \"tmp/files/tmp.wav\"")
